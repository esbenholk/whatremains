...
function display_video_meta_box( $video_post ) {
    ...
    <table>
        <tr>
            <td style="width: 100%">Is it currently featured?, input "feature" to feature film and "has been featured" to take it off the program again. any other input means the film has yet to be featured </td>
        ...
        <tr>
            ...
            <td> <input type="text" name="the_blurb" id="the_blurb" placeholder="video id" value="<?php echo $the_blurb?>"/></td>  
        </tr>  
        <tr>
            <td style="width: 150px">Trailer Link copied from Vimeo embed</td>
            <td> <input type="url" name="video_url" id="url" placeholder="video id"  pattern="https://player.vimeo.com/video/.+"value="<?php echo $video_url?>"/></td>  
        </tr>  
        <tr>
            <td style="width: 150px">Date (for sorting in program) *required!!!!! otherwise the item wont show in the program</td>
            <td><input type="date" size="80" name="specific_date" value="<?php echo $specific_date ?>" /></td>
        </tr>  
    </table>
    <?php
}
...
function display_whatremains_landingpage_meta_box( $page ) {
        ...
        ?>
            ...
            <tr>
                <td style="width: 100%">Show featured trailer</td>    
            ...
            <p>all the below fields should be updated each 2 weeks to fit the new cycle of featured films. In addition to updating the trailer on the front page and the phase info, dont forget to visit each video to check and uncheck if it is featured or not</p>
            <tr>
                <td style="width: 150px">Trailer Link (ID Number from Vimeo: check end of url when on video)</td>
...
function display_talk_meta_box( $video_post ) {
    // Retrieve current name of the Director and Movie Rating based on review ID
        ...
        <table>
            ...
            <tr>
                <td style="width: 150px">Date (for sorting in program)</td>
                <td><input type="date" size="80" name="specific_date" value="<?php echo $specific_date ?>" /></td>
            </tr>  
            ...
            <tr>
                <td style="width: 150px">Sign up Link</td>
...
function add_video_fields( $video_post_id, $video_post) {
    ...
    if($video_post->post_type == 'talk_post'){
        // Store data in post meta table if present in post data
            ...
            if ( isset( $_POST['sign_up_link'] ) && $_POST['sign_up_link'] != '' ) {
...
function include_template_function( $template_path ) {
    ...
    if ( is_page( 'what-remains-exhibition-text' ) ) {
...
function wpse_enqueue_page_template_styles() {
    if ( get_post_type() == 'video_post' || is_page( 'what-remains' ) || is_page( 'about-what-remains' ) || is_page( 'what-remains-exhibition-text' )   ) {
